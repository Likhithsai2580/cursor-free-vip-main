...
class Translator:
    def __init__(self):
        ...
        config_object, config_file_path = get_config()
        self.config = config_object
        self.config_file_path = config_file_path
        
        # Create language cache directory if it doesn't exist
        if self.config and self.config.has_section('Language') and self.config.has_option('Language', 'language_cache_dir'):
            self.language_cache_dir = self.config.get('Language', 'language_cache_dir')
            # Ensure this directory is also created, as setup_config might not cover it if Language section was added later
            if self.language_cache_dir:
                 os.makedirs(self.language_cache_dir, exist_ok=True)
        else:
            # If config is minimal or language_cache_dir is somehow missing, try to define a fallback or log an error
            # For now, setting to None, but this could be an issue for language caching.
            self.language_cache_dir = None 
            if self.config_file_path:
                # Attempt to set a default language_cache_dir if config_file_path is known
                # This assumes config_dir is the parent of config_file_path
                config_dir = os.path.dirname(self.config_file_path)
                if config_dir:
                    self.language_cache_dir = os.path.join(config_dir, "language_cache")
                    if self.config and not self.config.has_section('Language'):
                        self.config.add_section('Language')
                    if self.config:
                        self.config.set('Language', 'language_cache_dir', self.language_cache_dir)
                        # Note: This change to self.config might need to be saved back to disk
                    os.makedirs(self.language_cache_dir, exist_ok=True)
        ...
        if self.config and self.config.has_section('Language') and self.config.has_option('Language', 'current_language'):
            ...
            if saved_language and saved_language.strip():
                ...
                if self.config.has_section('Language'): # Should always be true if previous option was found
            ...
            if self.config:
                if not self.config.has_section('Language'):
                    self.config.add_section('Language')
                self.config.set('Language', 'current_language', self.current_language)
                # Ensure other default Language settings are present if section was new or incomplete
                if not self.config.has_option('Language', 'fallback_language'):
                    self.config.set('Language', 'fallback_language', 'en')
                if not self.config.has_option('Language', 'auto_update_languages'):
                    self.config.set('Language', 'auto_update_languages', 'True')
                if not self.config.has_option('Language', 'language_cache_dir') and self.language_cache_dir:
                    self.config.set('Language', 'language_cache_dir', self.language_cache_dir)
                language_needs_saving = True
        
        if language_needs_saving and self.config and self.config_file_path:
            try:
                with open(self.config_file_path, 'w', encoding='utf-8') as f:
                    self.config.write(f)
            except Exception as e:
                print(f"{Fore.RED}{EMOJI['ERROR']} Failed to save language settings to config: {e}{Style.RESET_ALL}")
...
def print_menu():
    ...
    try:
        ...
        if config_result:
            ...
            if config and config.has_section('Utils') and config.has_option('Utils', 'enabled_account_info'):
...
def select_language():
    ...
    try:
        ...
        if choice.isdigit() and 0 <= int(choice) < languages_count:
            ...
            config_result = get_config()
            if config_result:
                config, config_file_path = config_result
                if config and config.has_section('Language'):
                    config.set('Language', 'current_language', selected_language)
                    
                    # Use the config_file_path from get_config
                    if config_file_path and os.path.dirname(config_file_path):
                        try:
                            ...
                            print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('menu.config_save_failed', error=str(e))}{Style.RESET_ALL}")
                    else:
                        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('menu.config_path_not_found')}{Style.RESET_ALL}")
...
def main():
    ...
    config_result = get_config(translator)
    if not config_result:
        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('menu.config_init_failed')}{Style.RESET_ALL}")
        return
        
    config, config_path = config_result  # Unpack the tuple
    force_update_config(translator)

    if config and config.has_section('Utils') and config.has_option('Utils', 'enabled_update_check'):
