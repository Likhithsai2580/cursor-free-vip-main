...
def setup_driver(translator=None):
    ...
    try:
        ...
        if not config_result:
            print(f"{Fore.RED}❌ {translator.get('register.config_failed') if translator else 'Failed to load configuration'}{Style.RESET_ALL}")
...
def run_proxy(translator=None):
    """Run signup with proxy"""
    # Get configuration
    config_result = get_config(translator)
    if not config_result:
        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('signup.config_failed')}{Style.RESET_ALL}")
        return False
