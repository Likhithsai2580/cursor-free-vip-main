...
def get_user_documents_path():
    """Get user documents path with improved fallback mechanisms"""
    if platform.system() == "Windows":
        try:
            # Method 1: Try using SHGetFolderPath from ctypes
            try:
                ...
                print(f"SHGetFolderPath method failed: {e}")
            ...
            # Method 3: Try standard environment variables
            try:
                for env_var in ['USERPROFILE', 'HOMEPATH', 'HOME']:
                    if env_var in os.environ:
                        docs_path = os.path.join(os.environ[env_var], 'Documents')
                        if os.path.exists(docs_path):
                            return docs_path
            except Exception as e:
                print(f"Environment variable method failed: {e}")

            # Method 4: Use current user home directory + Documents
            try:
                ...
                print(f"Home directory method failed: {e}")
                
            # Last resort: Current directory
            print("Using current directory as fallback for Documents path")
            return os.path.abspath('.')
                
        except Exception as e:
            print(f"All document path detection methods failed: {e}")
            return os.path.abspath('.')
    else:
