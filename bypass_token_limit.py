...
def run(translator=None):
    ...
    print(f"\n{Fore.CYAN}{EMOJI['BYPASS']} {translator.get('token_limit.bypasstoken_limit') if translator else 'Bypass Token Limit'}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}{'─' * 70}{Style.RESET_ALL}")
    
    # Get paths from config
    config_result = get_config(translator)
    if not config_result:
        print(f"{Fore.RED}{EMOJI['ERROR']} {translator.get('token_limit.config_not_found') if translator else 'Configuration not found'}{Style.RESET_ALL}")
        return
        
    config, _ = config_result
    
    # Determine system and get paths
    system = platform.system()
    ...
    if system == "Linux":
        ...
        base_path = config.get(f"{system.upper()}Paths", "cursor_path")

    if not base_path:
        raise OSError(translator.get('reset.file_not_found', path=base_path) if translator else f"未找到 Cursor main.js 文件: {base_path}")

    main_path = os.path.join(base_path, "out/vs/workbench/workbench.desktop.main.js")
    
    if not os.path.exists(main_path):
        raise OSError(translator.get('reset.file_not_found', path=main_path) if translator else f"未找到 Cursor main.js 文件: {main_path}")

    modify_workbench_js(main_path, translator)
